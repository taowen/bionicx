...
output_dir="${1:?usage: tools/prepare-desktop-rootfs.sh OUTPUT_BUNDLE_DIR}"
...
canonical="$repo_dir/build/desktop-rootfs-bundle"
inputs=(
    "$repo_dir/examples/hello/build-bundle.sh"
    "$repo_dir/tools/build-android-glibc.sh"
    "$repo_dir/tools/build-desktop-rootfs.sh"
...
if [[ ! -f "$canonical/INPUT-ID" ]] || \
    ...
    case "$canonical/" in
        "$repo_dir/build/desktop-rootfs-bundle/") rm -rf -- "$canonical" ;;
        *) echo "refusing to replace unexpected path: $canonical" >&2; exit 2 ;;
    esac
    mkdir -p "$canonical"
    "$repo_dir/tools/build-desktop-rootfs.sh" "$canonical"
...
# 80+ MiB desktop runtime in the host workspace.

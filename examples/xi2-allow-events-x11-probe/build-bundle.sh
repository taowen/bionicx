...
output_dir="${1:-$repo_dir/build/xi2-allow-events-x11-probe-bundle}"
...
podman run --rm --network host --userns=keep-id \
    ...
    aarch64-linux-gnu-gcc -O2 -Wall -Wextra -Werror \
        examples/xi2-allow-events-x11-probe/xi2-allow-events-x11-probe.c \
        -o "${output_dir#"$repo_dir/"}/app/bin/xi2-allow-events-x11-probe" \
        -lXi -lX11
...
patchelf --set-interpreter "$interpreter" --set-rpath "$rpath" \
    "$output_dir/app/bin/xi2-allow-events-x11-probe"

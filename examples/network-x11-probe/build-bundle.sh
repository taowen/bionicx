...
output_dir="${1:-$repo_dir/build/network-x11-probe-bundle}"
...
podman run --rm --network host --userns=keep-id \
    ...
    aarch64-linux-gnu-gcc -O2 -Wall -Wextra -Werror \
        examples/network-x11-probe/network-x11-probe.c \
        -o "$container_output/app/bin/network-x11-probe" -lX11 -lresolv

"$repo_dir/tools/resolve-elf-deps.py" \
    --entry "$output_dir/app/bin/network-x11-probe" \
    --search-root "$output_dir/rootfs/usr/lib" \
    --json "$output_dir/network-x11-probe-dependency-closure.json"
echo "$output_dir"

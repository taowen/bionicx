...
bundle="${BIONICX_COMPOSITE_BUNDLE:-$repo_dir/build/composite-x11-probe-bundle}"

"$repo_dir/examples/composite-x11-probe/build-bundle.sh" "$bundle"
"$repo_dir/tools/install-profile.sh" \
    --profile "$repo_dir/profiles/composite-x11-probe.json" \
    --app-root "$bundle/app" \
    --serial "$serial"
...
for i in $(seq 1 30); do
    if adb -s "$serial" logcat -d -v brief \
            | grep -Fq 'BXSUMMARY composite-x11'; then
        echo "composite-x11 probe finished at ${i}s"
        break
    fi
    sleep 1
done

...
bundle="${BIONICX_XI1_BUNDLE:-$repo_dir/build/xi1-x11-probe-bundle}"

"$repo_dir/examples/xi1-x11-probe/build-bundle.sh" "$bundle"
"$repo_dir/tools/install-profile.sh" \
    --profile "$repo_dir/profiles/xi1-x11-probe.json" \
    --app-root "$bundle/app" \
    --serial "$serial"
...
for i in $(seq 1 30); do
    if adb -s "$serial" logcat -d -v brief \
            | grep -Fq 'BXSUMMARY xi1-x11'; then
        echo "xi1 probe finished at ${i}s"
        break
    fi
    sleep 1
done
...
printf '%s\n' "$result"
grep -Fq "BXSUMMARY xi1-x11 passed=4 failed=0" <<<"$result"
echo "XI1 X11 probe: PASS"

...
output_dir="${1:-$repo_dir/build/clipboard-manager-x11-probe-bundle}"
...
podman run --rm --network host --userns=keep-id \
    ...
    aarch64-linux-gnu-gcc -O2 -Wall -Wextra -Werror \
        examples/clipboard-manager-x11-probe/clipboard-manager-x11-probe.c \
        -o "${output_dir#"$repo_dir/"}/app/bin/clipboard-manager-x11-probe" \
        -lXfixes -lX11
...
patchelf --set-interpreter "$interpreter" --set-rpath "$rpath" \
    "$output_dir/app/bin/clipboard-manager-x11-probe"
printf 'required_package=libxfixes3\nrootfs_payload=none\n' \

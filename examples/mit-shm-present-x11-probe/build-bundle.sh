...
output_dir="${1:-$repo_dir/build/mit-shm-present-x11-probe-bundle}"
...
podman run --rm --network host --userns=keep-id \
    ...
    aarch64-linux-gnu-gcc -O2 -Wall -Wextra -Werror \
        examples/mit-shm-present-x11-probe/mit-shm-present-x11-probe.c \
        -o "${output_dir#"$repo_dir/"}/app/bin/mit-shm-present-x11-probe" \
        -lXext -lX11
...
patchelf --set-interpreter "$interpreter" --set-rpath "$rpath" \
    "$output_dir/app/bin/mit-shm-present-x11-probe"
printf 'required_package=libxext6\nrootfs_payload=none\n' \

...
output_dir="${1:-$repo_dir/build/xkb-set-controls-x11-probe-bundle}"
...
podman run --rm --network host --userns=keep-id \
    ...
    aarch64-linux-gnu-gcc -O2 -Wall -Wextra -Werror \
        examples/xkb-set-controls-x11-probe/xkb-set-controls-x11-probe.c \
        -o "${output_dir#"$repo_dir/"}/app/bin/xkb-set-controls-x11-probe" \
        -lX11
...
patchelf --set-interpreter "$interpreter" --set-rpath "$rpath" \
    "$output_dir/app/bin/xkb-set-controls-x11-probe"

...
output_dir="${1:-$repo_dir/build/gtk-textview-probe-bundle}"
...
podman run --rm --network host --userns=keep-id \
    ...
    aarch64-linux-gnu-gcc -O2 -Wall -Wextra -Werror \
        examples/gtk-textview-probe/gtk-textview-probe.c \
        -o "${output_dir#"$repo_dir/"}/app/bin/gtk-textview-probe" \
        -ldl -lX11
...
patchelf --set-interpreter "$interpreter" --set-rpath "$rpath" \
    "$output_dir/app/bin/gtk-textview-probe"

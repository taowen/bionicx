...
output_dir="${1:-$repo_dir/build/server-grab-x11-probe-bundle}"
...
podman run --rm --network host --userns=keep-id \
    ...
    "$builder_image" aarch64-linux-gnu-gcc -O2 -Wall -Wextra -Werror \
        examples/server-grab-x11-probe/server-grab-x11-probe.c \
        -o "$container_output/app/bin/server-grab-x11-probe" -lX11 -pthread

"$repo_dir/tools/resolve-elf-deps.py" \
    --entry "$output_dir/app/bin/server-grab-x11-probe" \
    --search-root "$output_dir/rootfs/usr/lib" \
    --json "$output_dir/server-grab-x11-probe-dependency-closure.json"
echo "$output_dir"

...
output_dir="${1:-$repo_dir/build/gtk-template-probe-bundle}"
...
podman run --rm --network host --userns=keep-id \
    ...
    aarch64-linux-gnu-gcc -O2 -Wall -Wextra -Werror \
        examples/gtk-template-probe/gtk-template-probe.c \
        -o "${output_dir#"$repo_dir/"}/app/bin/gtk-template-probe" -ldl
...
patchelf --set-interpreter "$interpreter" --set-rpath "$rpath" \
    "$output_dir/app/bin/gtk-template-probe"

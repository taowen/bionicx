...
output_dir="${1:-$repo_dir/build/randr-x11-probe-bundle}"
...
podman run --rm --network host --userns=keep-id \
    ...
    aarch64-linux-gnu-gcc -O2 -Wall -Wextra -Werror \
        examples/randr-x11-probe/randr-x11-probe.c \
        -o "${output_dir#"$repo_dir/"}/app/bin/randr-x11-probe" \
        -lXrandr -lX11
...
patchelf --set-interpreter "$interpreter" --set-rpath "$rpath" \
    "$output_dir/app/bin/randr-x11-probe"
printf 'required_package=libxrandr2\nrootfs_payload=none\n' \

...
device_fonts="files/apps/wps-office/usr/share/fonts/bionicx"
device_config="files/apps/wps-office/etc/fonts/fonts.conf"
device_cache="files/homes/wps-office/.cache/fontconfig-bionicx-v1"
...
"${adb[@]}" shell run-as "$package" mkdir -p "$device_fonts" \
    files/apps/wps-office/etc/fonts
for font in "${fonts[@]}"; do
    name="$(basename "$font")"
    temporary="/data/local/tmp/bionicx-$name"
    "${adb[@]}" push "$font" "$temporary" >/dev/null
    "${adb[@]}" shell run-as "$package" cp "$temporary" "$device_fonts/$name"
    "${adb[@]}" shell rm "$temporary"
done

temporary="/data/local/tmp/bionicx-fonts.conf"
"${adb[@]}" push "$repo_dir/examples/wps/fonts.conf" "$temporary" >/dev/null
"${adb[@]}" shell run-as "$package" cp "$temporary" "$device_config"
"${adb[@]}" shell rm "$temporary"

# Fontconfig caches directory contents by path.  Remove only the dedicated
# BionicX cache so the next WPS process must discover the newly installed
# files.  This is app-private and does not require root.
...
echo "installed and verified ${#fonts[@]} static Liberation fonts and BionicX aliases"

...
output_dir="${1:-$repo_dir/build/reparent-x11-probe-bundle}"
...
podman run --rm --network host --userns=keep-id \
    ...
    aarch64-linux-gnu-gcc -O2 -Wall -Wextra -Werror \
        examples/reparent-x11-probe/reparent-x11-probe.c \
        -o "${output_dir#"$repo_dir/"}/app/bin/reparent-x11-probe" \
        -lXfixes -lXi -lXtst -lX11
...
patchelf --set-interpreter "$interpreter" --set-rpath "$rpath" \
    "$output_dir/app/bin/reparent-x11-probe"

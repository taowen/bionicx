...
bundle="${BIONICX_XI_X11_BUNDLE:-$repo_dir/build/xi-x11-probe-bundle}"

"$repo_dir/examples/xi-x11-probe/build-bundle.sh" "$bundle"
"$repo_dir/tools/install-profile.sh" \
    --profile "$repo_dir/profiles/xi-x11-probe.json" \
    --app-root "$bundle/app" \
    --serial "$serial"
...
for i in $(seq 1 30); do
    if adb -s "$serial" logcat -d -v brief \
            | grep -Fq 'BXSUMMARY xi-x11'; then
        echo "xi-x11 probe finished at ${i}s"
        break
    fi
    sleep 1
done
...
printf '%s\n' "$result"
grep -Fq "BXSUMMARY xi-x11 passed=10 failed=0" <<<"$result"
echo "XI family probe: PASS"

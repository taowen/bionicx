...
`gtk_menu_popup_at_widget` maps a sized `GDK_WINDOW_TEMP`, XTEST clicks
a `GtkMenuButton`, and a `button-press-event` handler (the Applications
plugin path) pops a menu when button 1 is pressed with no Control.

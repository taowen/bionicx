...
output_dir="${1:-$repo_dir/build/gtk-desktop-icon-probe-bundle}"
...
podman run --rm --network host --userns=keep-id \
    ...
    aarch64-linux-gnu-gcc -O2 -Wall -Wextra -Werror \
        examples/gtk-desktop-icon-probe/gtk-desktop-icon-probe.c \
        -o "${output_dir#"$repo_dir/"}/app/bin/gtk-desktop-icon-probe" -ldl
...
patchelf --set-interpreter "$interpreter" --set-rpath "$rpath" \
    "$output_dir/app/bin/gtk-desktop-icon-probe"

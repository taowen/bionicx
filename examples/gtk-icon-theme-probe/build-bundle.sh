...
output_dir="${1:-$repo_dir/build/gtk-icon-theme-probe-bundle}"
...
podman run --rm --network host --userns=keep-id \
    ...
    aarch64-linux-gnu-gcc -O2 -Wall -Wextra -Werror \
        examples/gtk-icon-theme-probe/gtk-icon-theme-probe.c \
        -o "${output_dir#"$repo_dir/"}/app/bin/gtk-icon-theme-probe" -ldl
...
patchelf --set-interpreter "$interpreter" --set-rpath "$rpath" \
    "$output_dir/app/bin/gtk-icon-theme-probe"

...
output_dir="${1:-$repo_dir/build/image-text8-shape-probe-bundle}"
...
podman run --rm --network host --userns=keep-id \
    ...
    aarch64-linux-gnu-gcc -O2 -Wall -Wextra -Werror \
        examples/image-text8-shape-probe/image-text8-shape-probe.c \
        -o "${output_dir#"$repo_dir/"}/app/bin/image-text8-shape-probe" \
        -lXext -lX11
...
patchelf --set-interpreter "$interpreter" --set-rpath "$rpath" \
    "$output_dir/app/bin/image-text8-shape-probe"

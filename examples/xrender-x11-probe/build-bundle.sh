...
output_dir="${1:-$repo_dir/build/xrender-x11-probe-bundle}"
...
podman run --rm --network host --userns=keep-id \
    ...
    aarch64-linux-gnu-gcc -O2 -Wall -Wextra -Werror \
        examples/xrender-x11-probe/xrender-x11-probe.c \
        -o "${output_dir#"$repo_dir/"}/app/bin/xrender-x11-probe" \
        -lXrender -lX11
...
patchelf --set-interpreter "$interpreter" --set-rpath "$rpath" \
    "$output_dir/app/bin/xrender-x11-probe"
printf 'required_package=libxrender1\nrootfs_payload=none\n' \

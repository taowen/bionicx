...
output_dir="${1:-$repo_dir/build/gtk-gdk-grab-probe-bundle}"
...
podman run --rm --network host --userns=keep-id \
    ...
    aarch64-linux-gnu-gcc -O2 -Wall -Wextra -Werror \
        examples/gtk-gdk-grab-probe/gtk-gdk-grab-probe.c \
        -o "${output_dir#"$repo_dir/"}/app/bin/gtk-gdk-grab-probe" \
        -ldl -lX11 -lXi -lXtst
...
patchelf --set-interpreter "$interpreter" --set-rpath "$rpath" \
    "$output_dir/app/bin/gtk-gdk-grab-probe"

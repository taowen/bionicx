...
output_dir="${1:-$repo_dir/build/resize-screen-x11-probe-bundle}"
...
podman run --rm --network host --userns=keep-id \
    ...
    aarch64-linux-gnu-gcc -O2 -Wall -Wextra -Werror \
        examples/resize-screen-x11-probe/resize-screen-x11-probe.c \
        -o "${output_dir#"$repo_dir/"}/app/bin/resize-screen-x11-probe" -lX11
...
patchelf --set-interpreter "$interpreter" --set-rpath "$rpath" \
    "$output_dir/app/bin/resize-screen-x11-probe"

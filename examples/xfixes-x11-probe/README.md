...
requests, `ChangeSaveSet`, region combine, region transform
(`Set`/`Translate`/`Invert`/`Extents`/`CreateRegionFromWindow`),
applying a region as GC/Picture clip, and `SetWindowShapeRegion`
`ShapeInput`, including under `GrabServer`.

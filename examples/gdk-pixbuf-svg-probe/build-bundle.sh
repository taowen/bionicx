...
output_dir="${1:-$repo_dir/build/gdk-pixbuf-svg-probe-bundle}"
...
podman run --rm --network host --userns=keep-id \
    ...
    aarch64-linux-gnu-gcc -O2 -Wall -Wextra -Werror \
        examples/gdk-pixbuf-svg-probe/gdk-pixbuf-svg-probe.c \
        -o "${output_dir#"$repo_dir/"}/app/bin/gdk-pixbuf-svg-probe" -ldl
...
patchelf --set-interpreter "$interpreter" --set-rpath "$rpath" \
    "$output_dir/app/bin/gdk-pixbuf-svg-probe"
printf 'required_package=librsvg2-common\nrootfs_payload=none\n' \

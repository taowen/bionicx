...
output_dir="${1:-$repo_dir/build/runtime-probe-bundle}"
...
podman run --rm --network host \
    ...
    aarch64-linux-gnu-gcc -O2 -Wall -Wextra -Werror \
        examples/runtime-probe/runtime-probe.c \
        -o "$container_output/app/bin/runtime-probe" -lX11 -ldl -pthread -lrt -lm

"$repo_dir/tools/resolve-elf-deps.py" \
    --entry "$output_dir/app/bin/runtime-probe" \
    --search-root "$output_dir/rootfs/usr/lib" \
    --json "$output_dir/runtime-probe-dependency-closure.json"
echo "$output_dir"

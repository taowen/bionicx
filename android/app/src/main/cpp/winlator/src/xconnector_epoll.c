...
static int createServerSocket(const char* sockPath) {
    ...
    const bool abstract = sockPath[0] == '@';
    const char* name = abstract ? sockPath + 1 : sockPath;
    const size_t nameLength = strlen(name);
    if (nameLength + (abstract ? 1 : 0) >= sizeof(serverAddr.sun_path)) goto error;
    ...
    if (abstract) {
        ...
        strncpy(serverAddr.sun_path, name, sizeof(serverAddr.sun_path) - 1);
        unlink(serverAddr.sun_path);
        addrLength = (int)(offsetof(struct sockaddr_un, sun_path) + nameLength + 1);
    }

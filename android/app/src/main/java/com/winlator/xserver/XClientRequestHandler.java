...
public class XClientRequestHandler implements RequestHandler {
    private static final String TAG = "WinlatorXRequest";
    ...
    private boolean handleNormalRequest(XClient client, XInputStream inputStream, XOutputStream outputStream) throws IOException {
        ...
        if (client.xServer.grabManager.consumeWatchedRequest(client)) {
            Log.i("BionicX", "BXINFO grab-client-req client="
                    ...
                    + " opcode=" + (opcode & 0xff)
                    + " data=" + (requestData & 0xff)
                    + " seq=" + (client.getSequenceNumber() & 0xffff));
        }
        try {
            if (opcode < 0) {
                ...
                switch (opcode) {
                    ...
                    case ClientOpcodes.CHANGE_SAVE_SET:
                        try (XLock lock = client.xServer.lock(
                            ...
                            WindowRequests.changeSaveSet(client, inputStream,
                                    outputStream);
                        }
                        break;
                    ...
                    case ClientOpcodes.GRAB_BUTTON:
                        try (XLock lock = client.xServer.lock(XServer.Lockable.WINDOW_MANAGER, XServer.Lockable.INPUT_DEVICE, XServer.Lockable.CURSOR_MANAGER)) {
                            GrabRequests.grabButton(client, inputStream, outputStream);
                        }
                        break;
                    ...
                    case ClientOpcodes.GRAB_KEY:
                        try (XLock lock = client.xServer.lock(XServer.Lockable.WINDOW_MANAGER, XServer.Lockable.INPUT_DEVICE)) {
                            GrabRequests.grabKey(client, inputStream, outputStream);
                        }
                        break;
                    ...
                    case ClientOpcodes.POLY_TEXT8:
                        ...
                        break;
                    case ClientOpcodes.IMAGE_TEXT8:
                        try (XLock lock = client.xServer.lock(XServer.Lockable.DRAWABLE_MANAGER, XServer.Lockable.GRAPHIC_CONTEXT_MANAGER)) {
                            DrawRequests.imageText8(client, inputStream, outputStream);
                        }
                        break;
                    case ClientOpcodes.IMAGE_TEXT16:
                        try (XLock lock = client.xServer.lock(XServer.Lockable.DRAWABLE_MANAGER, XServer.Lockable.GRAPHIC_CONTEXT_MANAGER)) {
                            DrawRequests.imageText16(client, inputStream, outputStream);
                        }
                        break;

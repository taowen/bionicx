...
public class XConnectorEpoll {
    private static final String TAG = "WinlatorXConnector";
    ...
    private void handleExistingConnection(Object tag) {
        ...
        try {
            if (inputStream != null) {
                int readResult = inputStream.readMoreData(canReceiveAncillaryMessages);
                if (readResult > 0) {
                    ...
                    // socket. Keep it registered with epoll and try again.
                    Log.w(TAG, "transient read errno=" + (-readResult)
                            + " fd=" + client.fd);
                }
                else {
                    Log.e(TAG, "readMoreData=" + readResult + " fd=" + client.fd);

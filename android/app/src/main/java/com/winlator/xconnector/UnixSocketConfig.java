...
public class UnixSocketConfig {
    ...
    /** Creates a Linux abstract AF_UNIX address. '@' represents sun_path[0]=NUL. */
    public static UnixSocketConfig createAbstract(String name) {
        if (name == null || name.isEmpty()) throw new IllegalArgumentException("empty socket name");
        return new UnixSocketConfig(name.charAt(0) == '@' ? name : '@' + name);
    }
}

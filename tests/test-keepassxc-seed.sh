...
grep -F 'db-create' \
    "$repo_dir/examples/keepassxc/seed-db.sh" >/dev/null
grep -F 'db-add' \
    "$repo_dir/examples/keepassxc/seed-db.sh" >/dev/null
grep -F 'db-ls' \
    "$repo_dir/examples/keepassxc/seed-db.sh" >/dev/null
...
grep -F 'keepassxc-cli' \
    "$repo_dir/examples/keepassxc/seed-db.sh" >/dev/null
grep -F 'Version>2.0' \
    "$repo_dir/examples/keepassxc/fixtures/bionicx.key" >/dev/null
grep -F '630DCD29' \
    "$repo_dir/examples/keepassxc/fixtures/bionicx.key" >/dev/null
...
grep -F 'openDatabase' \
    "$repo_dir/examples/keepassxc/keepassxc-deferred-open" >/dev/null

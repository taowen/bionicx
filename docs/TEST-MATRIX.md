...
| Host GPU | `glx-probe` | Real glibc Xlib/GLX client through the pinned BionicX Gladio submodule; dynamic GLX opcode discovery; GLES 3 numeric/string capabilities, per-format MSAA, transform-feedback objects, integer vertex attributes and modern integer GLSL built-ins, BGRA render targets, and real program-binary export/import; desktop GL translation onto host GLES; GPU pixel readback; EGL share-group handoff into the Android X11 compositor | client 25/25 plus final Android screenshot pixel assertion pass on `/vendor/lib64/egl/libGLESv2_adreno.so`; Chrome passes ANGLE's GLES 3, program-cache, BGRA shared-image, integer-attribute, and complex Skia shader compilation; its GPU process remains alive but the Chrome surface is black, so final presentation remains pending; native Vulkan bridge and DRI3/Present `AHardwareBuffer` paths remain pending |

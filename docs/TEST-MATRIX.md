...
| Chromium | Google Chrome stable 151.0.7922.108 ARM64 | pinned/hash-locked package acquisition, apt/dpkg-installed shared Debian 13 trixie rootfs with WPS and IceWM, preserved package database/data/plugins and a `GLIBC_2.41` symbol-floor gate, direct patched-PT_INTERP bootstrap, ICU `/proc/self/exe` path, XKB partial-map parsing, Android seccomp feature probes, child Crashpad-policy propagation, Fontconfig, Android DNS bridge, deterministic 1920x1080 X11 window, ANGLE/Ganesh OpenGL on host GLES; separate ANGLE Vulkan profile through Vortek; sandbox and services | the 275-package system layer checks 1069 ELF objects and is transferred once by content ID; IceWM remains 4/4 and the normal Chrome profile reaches its complete 1920x1080 UI from that rootfs; it also loads Example Domain through DNS/TCP/TLS and renders a complete hardware-accelerated full-screen UI untraced under the ordinary app UID; ANGLE Vulkan creates its device, acquires the Queue2 graphics queue, creates an AHardwareBuffer swapchain, remains alive, renders the complete browser and internal GPU page without black regions, accepts Android-injected navigation, and reports hardware-accelerated Canvas/compositing/raster/WebGL/WebGPU on the Vortek Adreno Vulkan device; `--no-sandbox` is intentionally retained and remaining services are pending |
| office | real WPS 11.1.0.11720 ARM64 | apt/dpkg-installed shared trixie dependencies; Writer, Sheets, Presentation, PDF import/export, clipboard | from the shared 275-package rootfs Writer reaches its home screen, creates a real document and accepts editor input untraced; earlier Writer durable copy/paste/cold reopen, structurally verified PDF export, and post-export Open File all pass; Spreadsheets creates/calculates/saves/cold-reopens valid OOXML; Presentation edits, saves, structurally verifies, presents full-screen, exits via Escape, and cold-reopens valid PPTX untraced; WPS PDF opens a structurally checked two-page fixture, renders exact content, navigates, zooms, and cold-reopens untraced |

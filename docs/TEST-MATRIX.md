...
| Host GPU | `glx-probe`, `vulkan-probe` | Real glibc clients through pinned BionicX Gladio and Vortek submodules; complete GLX/GLES capability, shader, cache, draw, readback, and compositor coverage; optional app-private Vulkan host service, glibc Vulkan loader/ICD discovery, shared-ring RPC into a Bionic process, native vendor physical-device/queue/memory discovery, honest device-extension filtering, Xlib/XCB WSI translation, logical-device/swapchain commands, coherent mapped vertex/index/uniform upload, descriptor allocation/update/binding, SPIR-V graphics pipelines/render passes/indexed draws, present-semaphore ownership, and imported `AHardwareBuffer` presentation | GLX client 26/26 plus final Android screenshot pixel assertion pass; Chrome's normal untraced profile displays its complete UI through ANGLE/Ganesh OpenGL; Vulkan client 34/34 binds both Xlib and XCB surfaces to a real 640x360 X window, verifies unimplemented fragment shading rate is hidden, uploads interleaved vertices, indices and a uniform through dma-buf synchronization, binds the uniform descriptor, executes an indexed draw, consumes the present semaphore without a pre-present idle, and passes exact two-color Android screenshot assertions through `/vendor/lib64/hw/vulkan.adreno.so`; Chrome ANGLE Vulkan creates 68 observed graphics pipelines successfully, while sampled-image upload and multi-image lifecycle remain under investigation |
...
| Chromium | Google Chrome stable 151.0.7922.108 ARM64 | pinned/hash-locked package acquisition, direct patched-PT_INTERP bootstrap, recursive dependency closure plus declared runtime-loaded NSS roots, ICU `/proc/self/exe` path, XKB partial-map parsing, Android seccomp feature probes, child Crashpad-policy propagation, app-private Fontconfig plus Android/Liberation fonts, Android DNS bridge, deterministic 1920x1080 X11 window, ANGLE/Ganesh OpenGL on host GLES; separate ANGLE Vulkan profile through Vortek; sandbox and services | the normal profile loads Example Domain through DNS/TCP/TLS and renders a complete hardware-accelerated full-screen UI untraced under the ordinary app UID; ANGLE Vulkan now creates its device, acquires the Queue2 graphics queue, creates an AHardwareBuffer swapchain, remains alive, and displays a partial browser UI; pipeline-state serialization, sandbox, and remaining services are pending |

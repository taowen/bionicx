...
| Chromium | Google Chrome stable 151.0.7922.108 ARM64 | direct patched-PT_INTERP bootstrap, recursive dependency closure, ICU `/proc/self/exe` path, XKB partial-map parsing, Android seccomp feature probes, child Crashpad-policy propagation, X11 UI; sandbox, navigation/TLS, fonts, services and GPU/software rendering | first genuine browser window survives untraced under the ordinary app UID; child FD ownership/network-service restart loop is fixed; missing fonts and desktop services remain, and real navigation is not yet accepted |

...
| X11 core | `x11-probe`, `x11-window-tree`, `clipboard-x11-probe`, `keyboard-grab-x11-probe` | windows including legal `InputOnly` geometry, `InternAtom(only_if_exists)` returning `None`, cross-process recursive window observation, properties, drawing including GC-clipped `CopyArea`, cross-client UTF8 selection transfer and cleanup, async keyboard grab/contention/ungrab/owner-events/disconnect routing, events, modifier ordering | core 17/17, Chrome window tree with zero X errors, clipboard 5/5, and keyboard grab 7/7 strict checks pass with deterministic Android key/tap/swipe injection |
...
| Host GPU | `glx-probe` | Real glibc Xlib/GLX client through the pinned BionicX Gladio fork; dynamic GLX opcode discovery; GLES 3 numeric/string capabilities, per-format MSAA and transform-feedback objects; desktop GL translation onto host GLES; Qualcomm identity and GPU pixel readback; EGL share-group handoff into the Android X11 compositor | client 20/20 plus final Android screenshot pixel assertion pass on `/vendor/lib64/egl/libGLESv2_adreno.so`; Chrome now passes ANGLE's GLES 3 gate, with a later FD-lifecycle failure under diagnosis; native Mesa DRI3/Present `AHardwareBuffer` path remains pending |

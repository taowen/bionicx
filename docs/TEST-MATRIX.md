...
| Chromium | Google Chrome stable 151.0.7922.108 ARM64 | direct patched-PT_INTERP bootstrap, recursive dependency closure, ICU `/proc/self/exe` path, XKB partial-map parsing, Android seccomp feature probes, child Crashpad-policy propagation, app-private Fontconfig plus Android/Liberation fonts, X11 UI; sandbox, DNS/navigation/TLS, services and GPU/software rendering | genuine, fully legible browser and network-error UI survives untraced under the ordinary app UID; child FD ownership/network-service restart loop is fixed; navigation currently reaches `DNS_PROBE_FINISHED_BAD_CONFIG`, so Android DNS bridging is next |

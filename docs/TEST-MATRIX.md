...
| Host GPU | `glx-probe` | Real glibc Xlib/GLX client through the pinned BionicX Gladio fork; dynamic GLX opcode discovery; GLES 3 numeric/string capabilities, per-format MSAA, transform-feedback objects, and real program-binary export/import; desktop GL translation onto host GLES; Qualcomm identity and GPU pixel readback; EGL share-group handoff into the Android X11 compositor | client 21/21 plus final Android screenshot pixel assertion pass on `/vendor/lib64/egl/libGLESv2_adreno.so`; Chrome passes ANGLE's GLES 3 and program-cache startup, while its shared-image backing remains pending; native Vulkan bridge and DRI3/Present `AHardwareBuffer` paths remain pending |

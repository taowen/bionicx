...
| X11 core | `x11-probe`, `x11-window-tree`, `clipboard-x11-probe`, `keyboard-grab-x11-probe`, `pointer-grab-x11-probe`, `server-grab-x11-probe`, `save-set-x11-probe`, `icewm-probe` | windows including legal `InputOnly` geometry, `InternAtom(only_if_exists)` returning `None`, honest extension advertisement (incomplete DRI3 hidden), cross-process recursive window observation, property round trips and enumeration, cursor-font resource lifecycle, drawing including GC-clipped `CopyArea`, PolyPoint origin/previous coordinates and tiled `CWBackPixmap` clear semantics, cross-client UTF8 selection transfer and cleanup, async active/passive keyboard and passive pointer grabs with contention/ungrab/owner-events/disconnect/automatic-release routing and cursor override, exact pointer event state, server-grab request/setup deferral and disconnect cleanup, WM save-set rescue/reparent/map/coordinate preservation, events, modifier ordering; real WM root `SubstructureRedirect`, request-owner exclusion, reparent coordinates/events and nested mapping; declared runtime-loaded Imlib2 plugin closure | core 22/22, Chrome window tree with zero X errors, clipboard 5/5, keyboard grab 9/9, passive pointer grab 5/5, server grab 5/5, save set 2/2, and unmodified Debian ARM64 IceWM starting a mapped and visibly themed 1920x32 taskbar and managing two independently connected glibc clients 3/3 all pass with zero unsupported opcodes; IceWM's four synchronous-pointer `GrabButton` registrations still receive explicit BadImplementation, cursor-font glyph shapes are approximate, and panel text placement/client decorations remain incomplete |

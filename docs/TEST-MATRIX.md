...
| X11 core | `x11-probe`, `x11-window-tree`, `clipboard-x11-probe`, `keyboard-grab-x11-probe`, `pointer-grab-x11-probe`, `icewm-probe` | windows including legal `InputOnly` geometry, `InternAtom(only_if_exists)` returning `None`, honest extension advertisement (incomplete DRI3 hidden), cross-process recursive window observation, property round trips and enumeration, drawing including GC-clipped `CopyArea`, PolyPoint origin/previous coordinates and tiled `CWBackPixmap` clear semantics, cross-client UTF8 selection transfer and cleanup, async active/passive keyboard and passive pointer grabs with contention/ungrab/owner-events/disconnect/automatic-release routing, exact pointer event state, events, modifier ordering; real WM root `SubstructureRedirect`, request-owner exclusion, reparent coordinates/events and nested mapping; declared runtime-loaded Imlib2 plugin closure | core 21/21, Chrome window tree with zero X errors, clipboard 5/5, keyboard grab 9/9, passive pointer grab 4/4, and unmodified Debian ARM64 IceWM starting a mapped and visibly themed 1920x32 taskbar and managing two independently connected glibc clients 3/3 all pass; panel text placement and client decorations remain incomplete |
